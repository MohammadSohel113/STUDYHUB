...
def course(slug):
    ...
    # IMPORTANT:
    # get_progress() returns a dictionary.
    # course.html expects a number.
    progress_data = get_progress(
        session["user_id"],
        slug
    )

    progress_percent = progress_data["completed"]
...
def certificate(slug):
    ...
    # progress is a dictionary here,
    # so use the completed value.
