...
LANGUAGES = [
    ...
    ("JavaScript", "🌐", "Learn JavaScript and web development.", "javascript"),
...
@app.route("/course/<slug>/complete", methods=["POST"])
@login_required
...
def certificate(slug):
    ...
    # Certificate sirf passed course ke liye
    if not progress["exam_passed"]:
        flash(
            "You must pass the exam to get the certificate.",
            "error"
        )
    ...
    return render_template(
        ...
        certificate_date=date.today().strftime("%d %B %Y")
    )
